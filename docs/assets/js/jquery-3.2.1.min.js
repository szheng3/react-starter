...
!(function (a, b) {
  ...
  let c = [],
    ...
    map(a) { return this.pushStack(r.map(this, (b, c) => a.call(b, c, b))); },
    ...
    let b,
      ...
      da = function () { m(); },
      ea = ta(a => a.disabled === !0 && ('form' in a || 'label' in a), { dir: 'parentNode', next: 'legend' }); try { G.apply(D = H.call(v.childNodes), v.childNodes), D[v.childNodes.length].nodeType; } catch (fa) {
      G = {
      ...
      filter: {
        ...
        CLASS(a) { let b = y[`${a} `]; return b || (b = new RegExp(`(^|${K})${a}(${K}|$)`)) && y(a, a => b.test(typeof a.className === 'string' && a.className || typeof a.getAttribute !== 'undefined' && a.getAttribute('class') || '')); },
      ...
      pseudos: {
        ...
        has: ia(a => function (b) { return ga(a, b).length > 0; }),
        contains: ia(a => a = a.replace(_, aa), b => (b.textContent || b.innerText || e(b)).indexOf(a) > -1),
        lang: ia(a => U.test(a || '') || ga.error(`unsupported lang: ${a}`), a = a.replace(_, aa).toLowerCase(), b => { let c; do if (c = p ? b.lang : b.getAttribute('xml:lang') || b.getAttribute('lang')) return c = c.toLowerCase(), c === a || c.indexOf(`${a}-`) === 0; while ((b = b.parentNode) && b.nodeType === 1);return !1; }),
    ...
    r.cssHooks[b] = {
      get(a, c, d) { if (c) return !Qa.test(r.css(a, 'display')) || a.getClientRects().length && a.getBoundingClientRect().width ? $a(a, b, d) : ea(a, Sa, () => $a(a, b, d)); },
      set(a, c, d) {
        let e,
          f = d && Na(a),
  ...
  }); function _a(a, b, c, d, e) { return new _a.prototype.init(a, b, c, d, e); }r.Tween = _a, _a.prototype = {
    constructor: _a,
    init(a, b, c, d, e, f) { this.elem = a, this.prop = c, this.easing = e || r.easing._default, this.options = b, this.start = this.now = this.cur(), this.end = d, this.unit = f || (r.cssNumber[c] ? '' : 'px'); },
    cur() { const a = _a.propHooks[this.prop]; return a && a.get ? a.get(this) : _a.propHooks._default.get(this); },
    run(a) {
      let b,
    ...
    bb,
    cb = /^(?:toggle|show|hide)$/,
    db = /queueHooks$/; function eb() { bb && (d.hidden === !1 && a.requestAnimationFrame ? a.requestAnimationFrame(eb) : a.setTimeout(eb, r.fx.interval), r.fx.tick()); } function fb() { return a.setTimeout(() => { ab = void 0; }), ab = r.now(); } function gb(a, b) {
    let c,
      d = 0,
    ...
    var d,
      ...
      h = r.Deferred().always(() => { delete i.elem; }),
      ...
      j = h.promise({
        ...
        duration: c.duration,
        tweens: [],
        createTween(b, c) { const d = r.Tween(a, j.opts, b, c, j.opts.specialEasing[b] || j.opts.easing); return j.tweens.push(d), d; },
        stop(b) {
          let c = 0,
  ...
  }, (a, b) => { r.fn[a] = function (a, c, d) { return this.animate(b, a, c, d); }; }), r.timers = [], r.fx.tick = function () {
    let a,
      b = 0,
    ...
    mb = r.expr.attrHandle; r.fn.extend({ attr(a, b) { return T(this, r.attr, a, b, arguments.length > 1); }, removeAttr(a) { return this.each(function () { r.removeAttr(this, a); }); } }), r.extend({
    attr(a, b, c) {
      let d,
        e,
  ...
  }), lb = { set(a, b, c) { return b === !1 ? r.removeAttr(a, c) : a.setAttribute(c, c), c; } }, r.each(r.expr.match.bool.source.match(/\w+/g), (a, b) => {
    const c = mb[b] || r.find.attr; mb[b] = function (a, b, d) {
      let e,
        f,
        g = b.toLowerCase(); return d || (f = mb[g], mb[g] = e, e = c(a, b, d) != null ? g : null, mb[g] = f), e;
    };
  }); let nb = /^(?:input|select|textarea|button)$/i,
    ob = /^(?:a|area)$/i; r.fn.extend({ prop(a, b) { return T(this, r.prop, a, b, arguments.length > 1); }, removeProp(a) { return this.each(function () { delete this[r.propFix[a] || a]; }); } }), r.extend({
    prop(a, b, c) {
      let d,
        e,
    ...
    propHooks: { tabIndex: { get(a) { const b = r.find.attr(a, 'tabindex'); return b ? parseInt(b, 10) : nb.test(a.nodeName) || ob.test(a.nodeName) && a.href ? 0 : -1; } } },
  ...
  }); const rb = /\r/g; r.fn.extend({
    val(a) {
      let b,
        c,
        d,
        e = this[0]; { if (arguments.length) return d = r.isFunction(a), this.each(function (c) { let e; this.nodeType === 1 && (e = d ? a.call(this, c, r(this).val()) : a, e == null ? e = '' : typeof e === 'number' ? e += '' : Array.isArray(e) && (e = r.map(e, a => a == null ? '' : `${a}`)), b = r.valHooks[this.type] || r.valHooks[this.nodeName.toLowerCase()], b && 'set' in b && void 0 !== b.set(this, e, 'value') || (this.value = e)); }); if (e) return b = r.valHooks[e.type] || r.valHooks[e.nodeName.toLowerCase()], b && 'get' in b && void 0 !== (c = b.get(e, 'value')) ? c : (c = e.value, typeof c === 'string' ? c.replace(rb, '') : c == null ? '' : c); }
    }
  }), r.extend({
    valHooks: {
      option: { get(a) { const b = r.find.attr(a, 'value'); return b != null ? b : pb(r.text(a)); } },
      select: {
        get(a) {
          let b,
            c,
  ...
  }), r.each(['radio', 'checkbox'], function () { r.valHooks[this] = { set(a, b) { if (Array.isArray(b)) return a.checked = r.inArray(r(a).val(), b) > -1; } }, o.checkOn || (r.valHooks[this].get = function (a) { return a.getAttribute('value') === null ? 'on' : a.value; }); }); const sb = /^(?:focusinfocus|focusoutblur)$/; r.extend(r.event, {
    trigger(b, c, e, f) {
      let g,
        h,
        i,
    ...
    ub = r.now(),
    vb = /\?/; r.parseXML = function (b) { let c; if (!b || typeof b !== 'string') return null; try { c = (new a.DOMParser()).parseFromString(b, 'text/xml'); } catch (d) { c = void 0; } return c && !c.getElementsByTagName('parsererror').length || r.error(`Invalid XML: ${b}`), c; }; let wb = /\[\]$/,
    xb = /\r?\n/g,
    yb = /^(?:submit|button|image|reset|file)$/i,
    zb = /^(?:input|select|textarea|keygen)/i; function Ab(a, b, c, d) { let e; if (Array.isArray(b))r.each(b, (b, e) => { c || wb.test(a) ? d(a, e) : Ab(`${a}[${typeof e === 'object' && e != null ? b : ''}]`, e, c, d); }); else if (c || r.type(b) !== 'object')d(a, b); else for (e in b)Ab(`${a}[${e}]`, b[e], c, d); }r.param = function (a, b) {
    let c,
      d = [],
    ...
    let e = {},
      f = a === Jb; function g(h) { let i; return e[h] = !0, r.each(a[h] || [], (a, h) => { const j = h(b, c, d); return typeof j !== 'string' || f || e[j] ? f ? !(i = j) : void 0 : (b.dataTypes.unshift(j), g(j), !1); }), i; } return g(b.dataTypes[0]) || !e['*'] && g('*');
  } function Ob(a, b) {
    let c,
      d,
  ...
  }), r.expr.pseudos.hidden = function (a) { return !r.expr.pseudos.visible(a); }, r.expr.pseudos.visible = function (a) { return !!(a.offsetWidth || a.offsetHeight || a.getClientRects().length); }, r.ajaxSettings.xhr = function () { try { return new a.XMLHttpRequest(); } catch (b) {} }; let Rb = { 0: 200, 1223: 204 },
    Sb = r.ajaxSettings.xhr(); o.cors = !!Sb && 'withCredentials' in Sb, o.ajax = Sb = !!Sb, r.ajaxTransport(b => {
    let c,
  ...
  }), r.ajaxPrefilter(a => { a.crossDomain && (a.contents.script = !1); }), r.ajaxSetup({ accepts: { script: 'text/javascript, application/javascript, application/ecmascript, application/x-ecmascript' }, contents: { script: /\b(?:java|ecma)script\b/ }, converters: { 'text script': function (a) { return r.globalEval(a), a; } } }), r.ajaxPrefilter('script', a => { void 0 === a.cache && (a.cache = !1), a.crossDomain && (a.type = 'GET'); }), r.ajaxTransport('script', a => {
    if (a.crossDomain) {
      let b,
    ...
    Ub = /(=)\?(?=&|$)|\?\?/; r.ajaxSetup({ jsonp: 'callback', jsonpCallback() { const a = Tb.pop() || `${r.expando}_${ub++}`; return this[a] = !0, a; } }), r.ajaxPrefilter('json jsonp', (b, c, d) => {
    let e,
      f,
      g,
    ...
    let d,
      ...
      f,
      g = this,
      h = a.indexOf(' '); return h > -1 && (d = pb(a.slice(h)), a = a.slice(0, h)), r.isFunction(b) ? (c = b, b = void 0) : b && typeof b === 'object' && (e = 'POST'), g.length > 0 && r.ajax({
  ...
  }, r.each(['ajaxStart', 'ajaxStop', 'ajaxComplete', 'ajaxError', 'ajaxSuccess', 'ajaxSend'], (a, b) => { r.fn[b] = function (a) { return this.on(b, a); }; }), r.expr.pseudos.animated = function (a) { return r.grep(r.timers, b => a === b.elem).length; }, r.offset = {
    setOffset(a, b, c) {
      let d,
        e,
        f,
  ...
  }), r.holdReady = function (a) { a ? r.readyWait++ : r.ready(!0); }, r.isArray = Array.isArray, r.parseJSON = JSON.parse, r.nodeName = B, typeof define === 'function' && define.amd && define('jquery', [], () => r); let Vb = a.jQuery,
